        ...
        with TensorboardWriter(
            ...
            for update in range(self.config.NUM_UPDATES):
                ...
                if update % 10 == 0:
                    ...
                    writer.add_scalar("Policy/Learning_Rate", lr_scheduler.get_last_lr()[0], count_steps)

                    # ---- DMRN grad（若 update() 未清零 grad 则可记录）----
                    fusion = getattr(self.actor_critic.net, "fusion", None)
                    if fusion is not None:
                        g_a = getattr(fusion.beta_a, "grad", None)
                        g_v = getattr(fusion.beta_v, "grad", None)
                        if g_a is not None:
                            writer.add_scalar("DMRN/grad_beta_a", g_a.detach().abs().item(), count_steps)
                        if g_v is not None:
                            writer.add_scalar("DMRN/grad_beta_v", g_v.detach().abs().item(), count_steps)

                    # ---- DMRN 统计（由 policy.forward() 填充）----
                    net = self.actor_critic.net
                    if hasattr(net, "_last_dmrm_stats") and net._last_dmrm_stats:
                        for k, v in net._last_dmrm_stats.items():
                            writer.add_scalar(f"DMRN/{k}", v, count_steps)
